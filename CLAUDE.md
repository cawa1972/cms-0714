...
- The API requires an **authenticated user on every controller by default** (a global `AuthorizeFilter` in `Program.cs`). A new controller is protected automatically — you do **not** add `[Authorize]`. `AuthController` is the **only** `[AllowAnonymous]` one.
